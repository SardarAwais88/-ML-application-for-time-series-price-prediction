...
def main():
    ...
    if uploaded_file is not None:
        ...
        for col in ['Year', 'Month', 'Day', 'DayOfWeek']:
            ...
            input_features[0][col].number_input(f'Enter {col}', value=2023)

        # Check if the form is submitted
        if st.form_submit_button('Predict'):
            # Create a dictionary to store the input features
            input_data = {col: input_features[0][col].number_input(f'Enter {col}', value=2023) for col in ['Year', 'Month', 'Day', 'DayOfWeek']}
