...
def main():
    ...
    if uploaded_file is not None:
        ...
        input_features = st.form(key='prediction_form').columns(4)
        for col in ['Year', 'Month', 'Day', 'DayOfWeek']:
            # Use indexing to access each input element
            input_features[col][0].number_input(f'Enter {col}', value=2023)

        # Check if the form is submitted
        if st.form_submit_button('Predict'):
            # Create a dictionary to store the input features
            input_data = {col: input_features[col][0].number_input(f'Enter {col}', value=2023) for col in ['Year', 'Month', 'Day', 'DayOfWeek']}
            prediction = predict_price(trained_model, input_data)
